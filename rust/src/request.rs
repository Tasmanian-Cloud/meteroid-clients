...
impl Request {
    ...
    /// Execute the request and return raw binary data (for PDF downloads, etc.).
    ...
    /// Execute the request and return text data (for text/plain, text/html, etc.).
    pub async fn execute_text(self, conf: &Configuration) -> Result<String, Error> {
        match self.execute_with_backoff(conf).await? {
            None => Ok(String::new()),
            Some(bytes) => String::from_utf8(bytes.to_vec()).map_err(Error::generic),
        }
    }
    ...
    async fn execute_with_backoff(mut self, conf: &Configuration) -> Result<Option<Bytes>, Error> {
        ...
        request
            .headers_mut()
            .insert("meteroid-req-id", rand::rng().random::<u32>().into());
        ...
        loop {
            ...
            request
                .headers_mut()
                .insert("meteroid-retry-count", retry_count.into());
        }
    }
...
impl_query_param_value!(i64);
impl_query_param_value!(u32);
impl_query_param_value!(u64);
impl_query_param_value!(String);

impl<T: std::fmt::Display> QueryParamValue for Vec<T> {
    fn encode(&self) -> String {
